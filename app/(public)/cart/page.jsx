...
import { Plus, Minus, ShoppingBag, ChevronRight, LogIn } from "lucide-react";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-6">
          {cartItems.map((item) => (
            <div
              key={item.id}
              className="flex flex-col sm:flex-row items-center gap-6 bg-white border border-slate-100 p-6 rounded-[2rem] shadow-sm"
            >
              ...
              <div className="flex items-center bg-slate-50 rounded-2xl p-1 border border-slate-100 shadow-inner">
                <button
                  ...
                  className="w-8 h-8 flex items-center justify-center text-slate-500 hover:text-red-500 transition-all"
                >
                  <Minus size={16} />
                </button>
                <span className="px-4 text-sm font-black text-slate-900 min-w-[35px] text-center">
                  {item.quantity}
                </span>
                <button
                  onClick={() => addToCart(item)}
                  className="w-8 h-8 flex items-center justify-center bg-[#343A40] text-white rounded-xl active:scale-90"
                >
                  <Plus size={16} />
                </button>
              </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-slate-900 text-white p-8 rounded-[2.5rem] sticky top-28">
            ...
            <div className="space-y-4">
              ...
              <Link
                href="/cart/check"
                className="w-full bg-white text-slate-900 py-4 rounded-2xl mt-8 font-black uppercase text-sm hover:bg-blue-600 hover:text-white transition-all flex items-center justify-center gap-2 group"
              >
                Checkout Now{" "}
                <ChevronRight
                  size={18}
